...
function extractSkillFrequencies(items) {
  ...
  return Object.entries(counts)
    ...
    .slice(0, 20); // top 20 skills per role
}
...
async function scrapeRole(role, token) {
  ...
  // warmcache runs overnight so we can afford to wait longer
  let succeeded = false;
  for (let i = 0; i < 18; i++) {
    await sleep(5000);
    try {
  ...
  console.log(`  ✓ ${role}: ${totalJobs} jobs, ${skillFreq.length} skills extracted`);
...
module.exports = async function handler(req, res) {
  ...
  console.log(`Warmcache starting — ${ROLES.length} roles`);

  for (const role of ROLES) {
    const key = roleCacheKey(role);

    try {
      // Check if already cached and fresh (scraped within last 6 days)
      // This lets us re-run warmcache safely without wasting Apify credits
      ...
      if (existing?.scrapedAt) {
        ...
        if (age < sixDays) {
          results.skipped++;
          results.details.push({ role, status: 'skipped_fresh', scrapedAt: existing.scrapedAt });
          console.log(`  ⊙ ${role}: fresh cache, skipping`);
          await sleep(500); // small delay even on skip
          continue;
        }
        // Store previous week's frequencies for future trending
        // Kept in the new data object as prevFreq
        results.details.prevFreq = existing.skillFreq || [];
      }

      // Scrape fresh data
      const data = await scrapeRole(role, token);

      if (!data) {
        results.failed++;
        results.details.push({ role, status: 'failed' });
      } else {
        // Preserve previous week's data for trending
        const prevFreq = existing?.skillFreq || null;
        const toStore  = { ...data, prevFreq };

        const stored = await upstashSet(key, toStore);
        if (stored) {
          results.succeeded++;
          ...
          results.failed++;
          results.details.push({ role, status: 'storage_failed' });
          console.error(`  ✗ ${role}: Upstash storage failed`);
        }
      }

    } catch (e) {
      results.failed++;
      results.details.push({ role, status: 'error', error: e.message });
      console.error(`  ✗ ${role}: ${e.message}`);
    }

    // 3 second stagger between roles — avoids Apify rate limits
    await sleep(3000);
  }

  results.completed = new Date().toISOString();
  console.log(`Warmcache done: ${results.succeeded} warmed, ${results.skipped} skipped, ${results.failed} failed`);
